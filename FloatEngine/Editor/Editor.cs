...
namespace FloatEngine
{
    ...
    public partial class EditorWindow : Form
    {
        ...
        enum ObjectType
        {
            ...
            Enemy, PowerUp, Snow, NumOfObjects, //Every new objects have to go before NumObjects, because i < NumObjects
        };

        const string objectsNamespace = "FloatEngine."; //IMPORTANT: Type the namespace here that all of your classes will be in! Make sure you spell it exactly and put a . at the end!

        public EditorWindow(Game inputGame)
        {
            InitializeComponent();
            ...
            lightModeToolStripMenuItem.Text = "Light Mode (Selected)";
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            else if (mode == CreateMode.Decor)
            {
                ...
                newDecor.imagePath = "Art//decorplaceholder"; //Default image until we load the one we want.
        ...
        {
            grid = TextureLoader.Load("Art//128grid", content); //not a bad idea to replace with xyz arrows
            pixel = TextureLoader.Load("Art//pixel", content);
        }
        ...
        public void Update(List<GameObject> objects, Map map)
        {
                    ...
                    mode != CreateMode.Walls)
            {
                ...
                else if (Input.MouseLeftClicked() && mode == CreateMode.Models)
                {
                    for (int i = 0; i < map.decor.Count; i++)
                    {
                        if (map.decor[i].CheckCollision(new Rectangle(desiredIndex.X * map.tileSize, desiredIndex.Y * map.tileSize, 128, 128)))
            ...
            else if (Input.MouseLeftDown() == true && GameWindowFocused() == true)
            {
                ...
                else if (mode == CreateMode.Objects && placingItem == true)
                {
                    ...
                    SetListBox(game.map.decor, false);
                else if (mode == CreateMode.Models && placingItem == true)
                {
                    game.objects[game.objects.Count - 1].startPosition = game.objects[game.objects.Count - 1].position;
                    game.objects[game.objects.Count - 1].Initialize();
                    SetListBox(game.objects, false);
            ...
            else if (placingItem == true)
            {
                ...
                else if (mode == CreateMode.Models)
                    game.map.decor[game.map.decor.Count - 1].position = new Vector2(desiredIndex.X * map.tileSize, desiredIndex.Y * map.tileSize);
            }
        ...
        private void ResetEditorList()
        {
            ...
            objectsRadioButton.Checked = decorRadioButton.Checked = wallsRadioButton.Checked = false; // = modelsRadioButton.Checked 
        ...
        private void SaveAs()
        {
            ...
            saveFileDialog.Filter = "float (.float)|*.float";
        ...
        {
            darkModeToolStripMenuItem.Text = "Dark Mode (Selected)";
        ...
        {
            System.Windows.Forms.MessageBox.Show("FloatEngine (Version 0.3a)                                   " +
        ...
        public void OpenLevel()
        {
            ...
            openFileDialog1.Filter = "float (.float)|*.float";
